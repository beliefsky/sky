...
typedef struct {
    ...
    sky_size_t size;//已分配内存内中还未使用内存的大小
} sky_radix_tree_t;

//创建基数树，preallocate是预分配节点的个数
sky_radix_tree_t* sky_radix_tree_create(sky_pool_t* pool, sky_intptr_t preallocate);

//根据key值和掩码向基数树中插入value,返回值可能是OK 1,ERROR 0, BUSY -1
sky_bool_t sky_radix32tree_insert(sky_radix_tree_t* tree, sky_uint32_t key, sky_uint32_t mask, sky_uintptr_t value);

//根据key值和掩码删除节点（value的值）
sky_bool_t sky_radix32tree_delete(sky_radix_tree_t* tree, sky_uint32_t key, sky_uint32_t mask);

//根据key值在基数树中查找返回value数据
sky_uintptr_t sky_radix32tree_find(sky_radix_tree_t* tree, sky_uint32_t key);

sky_bool_t sky_radix128tree_insert(sky_radix_tree_t* tree, sky_uchar_t* key, sky_uchar_t* mask, sky_uintptr_t value);
