...
sky_inline void
sky_buf_init(sky_buf_t *buf, sky_pool_t *pool, sky_u32_t size) {
...
sky_buf_t*
sky_buf_create(sky_pool_t *pool, sky_u32_t size) {
    sky_buf_t *buf;
...
void sky_buf_rebuild(sky_buf_t *buf, sky_u32_t size) {
    const sky_u32_t n = (sky_u32_t) (buf->end - buf->pos);
    ...
    const sky_u32_t data_size = (sky_u32_t) (buf->last - buf->pos);
