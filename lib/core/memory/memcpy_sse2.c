...
static void memcpy_tiny(void *dst, const void *src, sky_usize_t size);
...
sky_memcpy(void *dst, const void *src, sky_usize_t n) {
    ...
    sky_uchar_t *post = dst;
    const sky_uchar_t *source = src;
    sky_usize_t padding = (16 - (((sky_usize_t) post) & 15)) & 15;

    if (padding > 0) {
        const __m128i head = _mm_loadu_si128((const __m128i *) source);
        _mm_storeu_si128((__m128i *) post, head);
        post += padding;
        source += padding;
        n -= padding;
    }

    if (n <= 0x200000) {  // L2-cache size
        __m128i c0, c1, c2, c3, c4, c5, c6, c7;

        for (; n >= 128; n -= 128) {
            c0 = _mm_loadu_si128(((const __m128i *) source) + 0);
            c1 = _mm_loadu_si128(((const __m128i *) source) + 1);
            c2 = _mm_loadu_si128(((const __m128i *) source) + 2);
            c3 = _mm_loadu_si128(((const __m128i *) source) + 3);
            c4 = _mm_loadu_si128(((const __m128i *) source) + 4);
            c5 = _mm_loadu_si128(((const __m128i *) source) + 5);
            c6 = _mm_loadu_si128(((const __m128i *) source) + 6);
            c7 = _mm_loadu_si128(((const __m128i *) source) + 7);
            _mm_prefetch((const sky_char_t *) (source + 256), _MM_HINT_NTA);
            source += 128;
            _mm_store_si128((((__m128i *) post) + 0), c0);
            _mm_store_si128((((__m128i *) post) + 1), c1);
            _mm_store_si128((((__m128i *) post) + 2), c2);
            _mm_store_si128((((__m128i *) post) + 3), c3);
            _mm_store_si128((((__m128i *) post) + 4), c4);
            _mm_store_si128((((__m128i *) post) + 5), c5);
            _mm_store_si128((((__m128i *) post) + 6), c6);
            _mm_store_si128((((__m128i *) post) + 7), c7);
            post += 128;
        }
    } else {        // big memory copy
        __m128i c0, c1, c2, c3, c4, c5, c6, c7;

        _mm_prefetch((const sky_char_t *) (source), _MM_HINT_NTA);

        if ((((sky_usize_t) source) & 15) == 0) {    // source aligned
            for (; n >= 128; n -= 128) {
                c0 = _mm_load_si128(((const __m128i *) source) + 0);
                c1 = _mm_load_si128(((const __m128i *) source) + 1);
                c2 = _mm_load_si128(((const __m128i *) source) + 2);
                c3 = _mm_load_si128(((const __m128i *) source) + 3);
                c4 = _mm_load_si128(((const __m128i *) source) + 4);
                c5 = _mm_load_si128(((const __m128i *) source) + 5);
                c6 = _mm_load_si128(((const __m128i *) source) + 6);
                c7 = _mm_load_si128(((const __m128i *) source) + 7);
                _mm_prefetch((const sky_char_t *) (source + 256), _MM_HINT_NTA);
                source += 128;
                _mm_stream_si128((((__m128i *) post) + 0), c0);
                _mm_stream_si128((((__m128i *) post) + 1), c1);
                _mm_stream_si128((((__m128i *) post) + 2), c2);
                _mm_stream_si128((((__m128i *) post) + 3), c3);
                _mm_stream_si128((((__m128i *) post) + 4), c4);
                _mm_stream_si128((((__m128i *) post) + 5), c5);
                _mm_stream_si128((((__m128i *) post) + 6), c6);
                _mm_stream_si128((((__m128i *) post) + 7), c7);
                post += 128;
            }
    ...
    memcpy_tiny(post, source, n);
}

static sky_inline void
memcpy_tiny(void *dst, const void *src, sky_usize_t size) {
    sky_uchar_t *dd = ((sky_uchar_t *) dst) + size;
    const sky_uchar_t *ss = ((sky_uchar_t *) src) + size;
    ...
    switch (size) {
        ...
        case 2:
            sky_memcpy2(dd - 2,ss - 2);
            break;

        case 67:
            memcpy_sse2_64(dd - 67, ss - 67);
        case 3:
            sky_memcpy2(dd - 3,ss - 3);
            dd[-1] = ss[-1];
            break;
        ...
        case 4:
            sky_memcpy4(dd - 4,ss - 4);
            break;

        case 69:
            memcpy_sse2_64(dd - 69, ss - 69);
        case 5:
            sky_memcpy4(dd - 5,ss - 5);
            dd[-1] = ss[-1];
            break;
        ...
        case 8:
            sky_memcpy8(dd - 8,ss - 8);
            break;

        case 73:
            memcpy_sse2_64(dd - 73, ss - 73);
        case 9:
            sky_memcpy8(dd - 9,ss - 9);
            dd[-1] = ss[-1];
            break;
...
memcpy_sse2_16(void *dst, const void *src) {
    const __m128i m0 = _mm_loadu_si128(((const __m128i *) src) + 0);
...
memcpy_sse2_32(void *dst, const void *src) {
    const __m128i m0 = _mm_loadu_si128(((const __m128i *) src) + 0);
    const __m128i m1 = _mm_loadu_si128(((const __m128i *) src) + 1);
    _mm_storeu_si128(((__m128i *) dst) + 0, m0);
...
memcpy_sse2_64(void *dst, const void *src) {
    const __m128i m0 = _mm_loadu_si128(((const __m128i *) src) + 0);
    const __m128i m1 = _mm_loadu_si128(((const __m128i *) src) + 1);
    const __m128i m2 = _mm_loadu_si128(((const __m128i *) src) + 2);
    const __m128i m3 = _mm_loadu_si128(((const __m128i *) src) + 3);
    _mm_storeu_si128(((__m128i *) dst) + 0, m0);
    _mm_storeu_si128(((__m128i *) dst) + 1, m1);
...
memcpy_sse2_128(void *dst, const void *src) {
    ...
    const __m128i m2 = _mm_loadu_si128(((const __m128i *) src) + 2);
    const __m128i m3 = _mm_loadu_si128(((const __m128i *) src) + 3);
    const __m128i m4 = _mm_loadu_si128(((const __m128i *) src) + 4);
    const __m128i m5 = _mm_loadu_si128(((const __m128i *) src) + 5);
    const __m128i m6 = _mm_loadu_si128(((const __m128i *) src) + 6);
    const __m128i m7 = _mm_loadu_si128(((const __m128i *) src) + 7);
    _mm_storeu_si128(((__m128i *) dst) + 0, m0);
    _mm_storeu_si128(((__m128i *) dst) + 1, m1);
    _mm_storeu_si128(((__m128i *) dst) + 2, m2);
    _mm_storeu_si128(((__m128i *) dst) + 3, m3);
