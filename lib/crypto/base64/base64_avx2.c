...
sky_api sky_usize_t
sky_base64_encode(sky_uchar_t *dest, const sky_uchar_t *str, size_t len) {
    if (sky_unlikely(!len)) {
        return 0;
    }
    ...
    if (len >= 32 - 4) {
        ...
        __m256i input_vector = _mm256_maskload_epi32((sky_i32_t const *) (str - 4), _mm256_set_epi32(
                (sky_i32_t) 0x80000000,
                (sky_i32_t) 0x80000000,
                (sky_i32_t) 0x80000000,
                (sky_i32_t) 0x80000000,
        ...
        ////////////
        for (;;) {
            input_vector = enc_reshuffle(input_vector);
            input_vector = enc_translate(input_vector);
            _mm256_storeu_si256((__m256i *) dest, input_vector);
            str += 24;
            dest += 32;
            len -= 24;
            if (len >= 32) {
                input_vector = _mm256_loadu_si256((__m256i *) (str - 4)); // no need for a mask here
    ...
    const sky_usize_t size = tables_base64_encode(dest, str, len);
    if (size == SKY_USIZE_MAX) {
        return SKY_USIZE_MAX;
    }
    return ((sky_usize_t) (dest - dest_orig)) + size;
}

sky_api sky_usize_t
sky_base64_decode(sky_uchar_t *out, const sky_uchar_t *src, sky_usize_t len) {
    if (sky_unlikely(!len)) {
        return 0;
    }

    sky_uchar_t *out_orig = out;
    while (len >= 45) {
    ...
    sky_usize_t size = tables_base64_decode(out, src, len);
    if (size == SKY_USIZE_MAX) {
        return SKY_USIZE_MAX;
    }
