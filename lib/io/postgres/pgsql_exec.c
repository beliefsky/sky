...
static void pgsql_exec_on_close(sky_tcp_t *tcp);
...
sky_pgsql_exec(
    ...
    sky_tcp_set_write_cb(&conn->tcp, pgsql_exec_send);
...
pgsql_exec_send(sky_tcp_t *const tcp) {
    ...
    do {
        ...
        if (n == SKY_USIZE_MAX) {
            ...
            sky_tcp_close(tcp, pgsql_exec_on_close);
            return;
        }
        if (!n) {
            return;
    ...
    sky_tcp_set_read_cb(tcp, pgsql_exec_read);
    pgsql_exec_read(tcp);
}
...
pgsql_exec_timeout(sky_timer_wheel_entry_t *const timer) {
    ...
    sky_tcp_close(&conn->tcp, pgsql_exec_on_close);
}

static void
pgsql_exec_on_close(sky_tcp_t *tcp) {
    sky_pgsql_conn_t *const conn = sky_type_convert(tcp, sky_pgsql_conn_t, tcp);
    conn->exec_cb(conn, null, conn->cb_data);
}