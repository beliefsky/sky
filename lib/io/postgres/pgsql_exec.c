...
static void on_pgsql_exec_send(sky_tcp_cli_t *tcp, sky_usize_t size, void *attr);
...
sky_pgsql_exec(
    ...
    switch (sky_tcp_write(
        ...
        case REQ_PENDING:
            return;
        case REQ_SUCCESS:
            on_pgsql_exec_send(&conn->tcp, bytes, null);
            return;
        default:
            sky_buf_destroy(buf);
...
static void
on_pgsql_exec_send(sky_tcp_cli_t *const tcp, sky_usize_t size, void *attr) {
    (void) attr;
    ...
    switch (sky_tcp_read(
        ...
        case REQ_PENDING:
            return;
        case REQ_SUCCESS:
            on_pgsql_exec_read(tcp, size, null);
            return;
        default:
            sky_buf_destroy(buf);
            sky_timer_wheel_unlink(&conn->timer);
...
static void
on_pgsql_exec_read(sky_tcp_cli_t *const tcp, sky_usize_t size, void *attr) {
    (void) attr;
...
//                sky_log_info("COMPLETE(%lu): %s", size, buf->pos);
                buf->pos += packet->size;
                packet->status = START;
                continue;
            }
...
//                sky_log_info("READY(%lu): %s", size, buf->pos);
                buf->pos += packet->size;
