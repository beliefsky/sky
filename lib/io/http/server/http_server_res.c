...
sky_http_response_nobody(
    ...
    sky_tcp_set_write_cb(&conn->tcp, http_response_str);
...
sky_http_response_str_len(
    ...
    packet->vec[0].len = result.len;
    packet->vec[1].buf = (sky_uchar_t *) data;
    packet->vec[1].len = data_len;
    ...
    sky_tcp_set_write_cb(&conn->tcp, http_response_vec);
