...
typedef struct {
    sky_rb_node_t node;
    sky_queue_t link;
    sky_queue_t open_queue;
    sky_fs_stat_t fs_stat;
    sky_str_t path;
    sky_fs_t file;
    sky_i64_t expire_at;
...
static void http_response(http_file_t *file);
...
static void on_file_open(sky_fs_t *fs, sky_bool_t success, void *data);

static void on_file_node_close(sky_fs_t *fs, void *data);


sky_api sky_http_server_module_t *
sky_http_server_file_create(sky_ev_loop_t *const ev_loop, const sky_http_server_file_conf_t *const conf) {
    ...
    sky_ev_timeout_init(ev_loop, &data->timer, cache_node_free_timer);
...
http_run_handler(sky_http_request_t *const r, void *const data) {
    ...
    file->req = r;
    file->node = node;
    file->module_file = module_file;

    if (sky_fs_opening(&node->file)) {
        sky_queue_init_node(&file->link);
        sky_queue_insert_next(&node->open_queue, &file->link);
        return;
    }
    if (node->not_open || sky_fs_status_is_dir(&node->fs_stat)) {
...
static void
http_response(http_file_t *file) {
    sky_http_request_t *const r = file->req;
    file_cache_node_t *const node = file->node;

    http_module_file_t *const module_file = file->module_file;
    sky_str_t *const exten = sky_http_req_exten(r);
    ...
    if (file->modified && file->modified_time == node->fs_stat.modified_time_sec) {
    ...
    header->val.len = sky_date_to_rfc_str(node->fs_stat.modified_time_sec, header->val.data);

    if (file->range && (!file->if_range || file->range_time == node->fs_stat.modified_time_sec)) {
        sky_http_res_set_status(r, 206);
        if (file->right == 0 || file->right > node->fs_stat.size) {
            file->right = node->fs_stat.size - 1;
        }
    } else {
        file->left = 0;
        file->right = node->fs_stat.size - 1;
...
cache_node_file_get_ref(http_module_file_t *module_file, sky_pool_t *pool, const sky_str_t *uri_path) {
    ...
    sky_fs_init(&node->file, module_file->ev_loop);
    sky_queue_init_node(&node->link);
    sky_queue_init(&node->open_queue);
    ...
    const sky_usize_t path_size = module_file->path.len + uri_path->len + 1;
    sky_uchar_t *const path = sky_palloc(pool, path_size);
    ...
    switch (sky_fs_open(
        ...
        case REQ_PENDING:
            break;
        case REQ_SUCCESS:
            node->not_open = !sky_fs_stat(&node->file, &node->fs_stat);
            break;
        default:
            node->not_open = true;
            break;
    }
    sky_pfree(pool, path, path_size);
...
cache_node_file_unref(file_cache_node_t *const node) {
    ...
    node->expire_at = sky_ev_now_sec(module_file->ev_loop) + module_file->cache_sec;
    sky_queue_insert_prev(&module_file->cache_queue, &node->link);
    if (!sky_timer_linked(&module_file->timer)) {
        sky_event_timeout_set(module_file->ev_loop, &module_file->timer, module_file->cache_sec * 1000);
    }
}
...
cache_node_free_timer(sky_timer_wheel_entry_t *const timer) {
    ...
    sky_queue_t *item;
    file_cache_node_t *node;
    const sky_i64_t now_time = sky_ev_now_sec(module_file->ev_loop);
    while (!sky_queue_empty(&module_file->cache_queue)) {
        ...
        if (node->expire_at > now_time) {
            sky_event_timeout_set(module_file->ev_loop, &module_file->timer,
                                  (sky_u32_t) (node->expire_at - now_time) * 1000);
            return;
        }
...
http_header_range(http_file_t *const file, const sky_str_t *const value) {
    ...
    if (sky_unlikely(index == -1)) {
        ...
        if (sky_unlikely(!sky_str_len_to_u64(start, (sky_usize_t) index, &file->left))) {
            return false;
        }
    }
    ...
    start += index;
    if (size > 0) {
        if (sky_unlikely(!sky_str_len_to_u64(start, size, &file->right))) {
            return false;
        }
    }
...
static void
on_file_open(sky_fs_t *const fs, sky_bool_t success, void *const data) {
    (void) data;
    ...
    if (success) {
        node->not_open = !sky_fs_stat(&node->file, &node->fs_stat);
        if (!node->not_open && !sky_fs_status_is_dir(&node->fs_stat)) {
            while (!sky_queue_empty(&node->open_queue)) {
                item = sky_queue_next(&node->open_queue);
                sky_queue_remove(item);
                file = sky_type_convert(item, http_file_t, link);
                http_response(file);
            }
            return;
        }
    ...
    while (!sky_queue_empty(&node->open_queue)) {
        ...
        file = sky_type_convert(item, http_file_t, link);
        cache_node_file_unref(node);
        http_error_page(file->req, 404, "404 Not Found");
    }
}
