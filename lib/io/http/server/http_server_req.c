...
sky_http_req_finish(sky_http_request_t *const r) {
    ...
    if (sky_unlikely(!r->response)) { //如果未响应则响应空数据
        ...
        sky_http_res_str_len(r, null, 0, null, null);
        return;
    }
...
http_module_run(sky_http_request_t *const r) {
    ...
    sky_http_res_str_len(r, sky_str_line("404 Not Found"), null, null);
}
...
http_timeout_cb(sky_timer_wheel_entry_t *const timer) {
    ...
    sky_tcp_cli_close(&conn->tcp, on_http_conn_close, null);
}
...
static void
on_http_conn_close(sky_tcp_cli_t *cli, void *data) {
    (void) data;
