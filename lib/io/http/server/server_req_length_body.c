...
static void http_res_too_large(sky_http_server_request_t * r, void * data);

static void on_http_body_none(sky_tcp_t *tcp, sky_tcp_req_t *req, sky_usize_t size);

static void on_http_body_too_large(sky_tcp_t *tcp, sky_tcp_req_t *req, sky_usize_t size);
...
static void
http_res_too_large(sky_http_server_request_t *const r, void *const data) {
    sky_http_connection_t *const conn = r->conn;

    if (r->error) {
        conn->next_str_cb(r, null, data);
        return;
    }
    sky_buf_t *const tmp = conn->buf;
    const sky_usize_t read_n = (sky_usize_t) (tmp->last - tmp->pos);
...
static void
on_http_body_too_large(sky_tcp_t *tcp, sky_tcp_req_t *req, sky_usize_t size) {
    (void )req;
