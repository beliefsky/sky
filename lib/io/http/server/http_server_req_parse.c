    ...
    switch (state) {
        ...
        case sw_method: {
            ...
            *(p++) = '\0';
            ...
            r->req_pos = p;
            state = sw_uri_no_decode;
        }
        case sw_uri_no_decode: {
            index = parse_url_no_decode(r, p, end);
        ...
        case sw_args_no_decode: {
            index = advance_token_no_decode(p, end);
            if (sky_unlikely(index < 0)) {
                if (sky_unlikely(index == -2)) {
                    return -1;
                }
...
http_method_identify(sky_http_server_request_t *const r) {
    ...
    switch (sky_str4_switch(m)) {
        ...
        case sky_str4_num('O', 'P', 'T', 'I'):
            ...
            break;
        case sky_str4_num('C', 'O', 'N', 'N'):
            m += 4;
            if (sky_unlikely(!sky_str4_cmp(m, 'E', 'C', 'T', '\0'))) {
                return false;
            }
            r->method = SKY_HTTP_CONNECT;
            break;
        default:
            return false;
    }
